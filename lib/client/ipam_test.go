...
var _ = Describe("IPAM tests", func() {
	...
	// assigned IP to be from the new ipPool that was created, this is to make sure the assigned IP
	// doesn't come from the old affinedBlock even after the ipPool was deleted.
	DescribeTable("IPAM AutoAssign from the default pool then delete the pool and assign again",
		func(config *api.CalicoAPIConfig) {
			...
			Context("AutoAssign 1 IP without specifying a pool", func() {
				...
				blocks := getAffineBlocks(host, config)
				...
				Expect(outErr).NotTo(HaveOccurred(), "assigned IP should be from pool1")
				Expect(pool1.IPNet.Contains(v4[0].IP)).To(BeTrue(), "assigned IP should be from pool1")
			})
			...
			Context("AutoAssign 1 IP without specifying a pool", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "assigned IP should be from pool1")
				Expect(block.IPNet.Contains(v4[0].IP)).To(BeTrue(), "assigned IP should be from pool1")
			})
			...
			Context("Delete pool1", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should delete the ipPool without any error")
			})
			...
			Context("AutoAssign 1 IP without specifying a pool", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "assigned IP should be from pool2")
				Expect(pool2.IPNet.Contains(v4[0].IP)).To(BeTrue(), "assigned IP should be from pool2")
			})
		},
		testutils.GetEmptyEntriesWithConfigs()...,
	)

	DescribeTable("IPAM AutoAssign from any pool",
		func(config *api.CalicoAPIConfig) {
			testutils.CleanBackend(config)
			c, _ := testutils.NewClient(config)
			ic := setupIPAMClient(c, true)
			...
			Context("AutoAssign 1 IP from any pool", func() {
				...
				v4, _, outErr := ic.AutoAssign(args)
				Expect(outErr).NotTo(HaveOccurred(), "should have assigned an IP address with no error")
				Expect(len(v4) == 1).To(BeTrue(), "should have assigned an IP address with no error")
	...
	DescribeTable("IPAM AutoAssign from different pools",
		func(config *api.CalicoAPIConfig) {
			testutils.CleanBackend(config)
			c, _ := testutils.NewClient(config)
			ic := setupIPAMClient(c, true)
			...
			Context("AutoAssign 1 IP from pool1", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should be from pool1")
				Expect(pool1.IPNet.Contains(v4[0].IP)).To(BeTrue(), "should be from pool1")
			})
			...
			Context("AutoAssign 1 IP from pool2", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should be from pool2")
				Expect(block2.IPNet.Contains(v4[0].IP)).To(BeTrue(), "should be from pool2")
			})
			...
			Context("AutoAssign 1 IP from pool1 (second time)", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should be a from the same block as the first IP from pool1")
				Expect(block1.IPNet.Contains(v4[0].IP)).To(BeTrue(), "should be a from the same block as the first IP from pool1")
			})
			...
			Context("AutoAssign 1 IP from pool2 (second time)", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should be a from the same block as the first IP pool2")
				Expect(block2.IPNet.Contains(v4[0].IP)).To(BeTrue(), "should be a from the same block as the first IP pool2")
			})
		},
		testutils.GetEmptyEntriesWithConfigs()...,
	)

	DescribeTable("IPAM AutoAssign from different pools - multi",
		func(config *api.CalicoAPIConfig) {
			testutils.CleanBackend(config)
			c, _ := testutils.NewClient(config)
			ic := setupIPAMClient(c, true)
			...
			Context("AutoAssign 300 IPs from 2 pools", func() {
				...
				Expect(outErr).NotTo(HaveOccurred(), "should not have failed")
				Expect(len(v4)).To(Equal(300), "should not have failed")
			})
			...
			Context("AutoAssign 300 IPs from both pools - none left tho", func() {
				...
				Expect(len(v4)).NotTo(Equal(300), "should have failed with less than 300")
				Expect(outErr).NotTo(HaveOccurred(), "should have failed with less than 300")
			})
		},
		testutils.GetEmptyEntriesWithConfigs()...,
	)
	...
	DescribeTable("AutoAssign: requested IPs vs returned IPs",
		func(host string, cleanEnv bool, pool []string, usePool string, inv4, inv6, expv4, expv6 int, expError error, config *api.CalicoAPIConfig) {
	...
	DescribeTable("AssignIP: requested IP vs returned error",
		func(inIP net.IP, host string, cleanEnv bool, pool []string, expError error, config *api.CalicoAPIConfig) {
	...
	DescribeTable("ReleaseIPs: requested IPs to be released vs actual unallocated IPs",
		func(inIP net.IP, cleanEnv bool, pool []string, assignIP net.IP, autoAssignNumIPv4 int, expUnallocatedIPs []cnet.IP, expError error, config *api.CalicoAPIConfig) {
	...
	DescribeTable("ClaimAffinity: claim IPNet vs actual number of blocks claimed",
		func(args testArgsClaimAff, config *api.CalicoAPIConfig) {
			inIPNet := testutils.MustParseCIDR(args.inNet)
			c, _ := testutils.NewClient(config)

			// Wipe clean backend, create a new client, and pools when cleanEnv flag is true.
			if args.cleanEnv {
				testutils.CleanBackend(config)
...
func getAffineBlocks(host string, config *api.CalicoAPIConfig) []cnet.IPNet {
	opts := model.BlockAffinityListOptions{Host: host, IPVersion: 4}
	compatClient, err := backend.NewClient(*config)
